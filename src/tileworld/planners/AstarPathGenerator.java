...
public class AstarPathGenerator implements TWPathGenerator {
    ...
    public TWPath findPath(int sx, int sy, int tx, int ty) {
        ...
        if (agent.getMemory().isCellBlocked(tx, ty, agent.getMemory().getSimulationTime()+agent.getMemory().distance(sx, sy, tx, ty))) {
            return null;
        }
        ...
        while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
            ...
            for (int x = -1; x < 2; x++) {
                for (int y = -1; y < 2; y++) {
                    ...
                    if (isValidLocation(sx, sy, xp, yp) && !agent.getMemory().isCellBlocked(xp, yp, agent.getMemory().getSimulationTime()+agent.getMemory().distance(sx, sy, xp, yp))) {
